...
const QmsPolicy = () => {
  ...
  const fetchPolicies = async () => {
    ...
    try {
      ...
      // Auto-expand if policies are found
      if (response.data.length > 0) {
        setIsExpanded(true);
      }
    } catch (error) {
      console.error("Error fetching policies:", error);
      setError("Failed to fetch policies. Please try refreshing the page.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="bg-[#1C1C24] rounded-lg text-white p-5">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {qmsPolicies.length === 0 && (
          <button
            ...
            disabled={isLoading}
          >
            <span>{isLoading ? "Loading..." : "Add Policy"}</span>
            {!isLoading && <Plus size={22} className='text-[#858585] group-hover:text-white transition-colors duration-200' />}
          </button>
        )}
      </div>
      ...
      <div className="bg-[#24242D] rounded-md overflow-hidden">
        ...
        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
          {isLoading ? (
            <div className="text-center p-5">Loading policies...</div>
            ...
            <div className="p-6">
              ...
              <div className="bg-[#1C1C24] p-5 rounded-md">
                {selectedPolicy.text && selectedPolicy.text.startsWith('<') ? (
                  ...
                  <p>{selectedPolicy.text || "No content available"}</p>
                )}
              </div>
